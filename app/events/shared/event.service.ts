...
export class EventService {
  ...
  getEvents(): Observable<Event[]> {
    return this.http.get('/api/events').map((response: Response) => response.json() as Event[]).catch(this.handleError);
  }
  ...
  saveEvent(event: Event): Observable<Event> {
    ...
    return this.http.post('/api/events', event, options).map((response: Response) => response.json() as Event).catch(this.handleError);
  }

  searchSessions(term: string): Observable<Sessions[]> {
    return this.http
      .get('/api/sessions/search?search=' + term)
      .map((response: Response) => {
        return response.json() as Sessions[];
      })
      .catch(this.handleError);
  }
...
const EVENTS: Event[] = [
  {
    ...
    sessions: [
      ...
      },
      {
        ...
        voters: ['johnpapa', 'bradgreen', 'igorminar', 'martinfowler', 'hicham1', 'ana1', 'hicham2', 'ana2', 'hicham3', 'ana3'],
      },
      {
        id: 3,
